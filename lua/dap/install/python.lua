...
function M.check_debugpy()
    ...
    vim.fn.jobstart(cmd_check, {
        on_exit = function(_, code)
            if code ~= 0 then
                notify.output_notify("debugpy is not installed", "Python Tools", "begin", "debugpy_cli", "debugpy", "warn")
                local cmd_install = cmd .. " -m pip install -i https://pypi.tuna.tsinghua.edu.cn/simple debugpy"
                -- 异步执行 cmd_install
                vim.fn.jobstart(cmd_install, {
                    on_exit = function(_, code)
                        if code == 0 then
                            notify.output_notify("debugpy install success", nil, "end", "debugpy_cli", "debugpy", "success")
                        else
...
function M.check_pyright()
    ...
    for i = 1, #commands do 
        ...
        vim.fn.jobstart(cmd .. " -m pip show "..commands[i], {
            on_exit = function(_, code)
                if code ~= 0 then
                    notify.output_notify(commands[i].." is not installed", "Python Tools", "begin", "debugpy_cli", commands[i], "warn")
                    local cmd_install = cmd .. " -m pip install -i https://pypi.tuna.tsinghua.edu.cn/simple pyright"
                    -- 异步执行 cmd_install
                    vim.fn.jobstart(cmd_install, {
                        on_exit = function(_, code)
                            if code == 0 then
                                notify.output_notify(commands[i].." install success", nil, "end", "debugpy_cli", commands[i], "success")
                                if commands[i] == "pyright" then
                                    require("dap.nvim-dap.python").setup()
                                end
                            else
                                notify.output_notify(commands[i].." install fail", nil, "end", "debugpy_cli", commands[i], "error")
                            end
                        end,
                    })
                end
