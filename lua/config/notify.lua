...
return function()
    ...
    vim.lsp.handlers["$/progress"] = function(_, result, ctx)
        ...
        if val.kind == "begin" then
            ...
            notify.output_notify(message, format_title(val.title, vim.lsp.get_client_by_id(client_id).name), val.kind,
        ...
        elseif val.kind == "end" and notif_data then
            notify.output_notify(val.message and format_message(val.message) or "Complete", nil, val.kind, client_id,
                result.token)
        end
    end
